#===============================================================================
#  LUTS::SpriteHash - wrapper for handling RMXP sprite hashes
#===============================================================================
module LUTS
  class SpriteHash
    #---------------------------------------------------------------------------
    ...
    def self.bitmap(path)
      ...
      return LUTS::Sprites::Bitmap.new(path.width, path.height) if path.is_a?(Rect)
      return path.bitmap if path.respond_to?(:bitmap) && path.bitmap

      bitmap = RPG::Cache.fromCache(path) || LUTS::Sprites::Bitmap.new(path)
    ...
    def sprite_instance(type = nil, klass = nil)
      ...
      return LUTS::Sprites::Base.new(@viewport) unless type

      "LUTS::Sprites::#{type.to_s.camelize}".constantize.new(@viewport)
    rescue NameError
      LUTS::ErrorMessages::SpriteError.new(type.to_s.camelize).raise
      LUTS::Sprites::Base.new(@viewport)
    end
    #---------------------------------------------------------------------------
